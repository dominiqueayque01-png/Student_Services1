...
function getClubById(id) { 
    return allClubsData.find(c => c._id === id); 
}
...
function openClubApplicationModal(clubId) {    
    currentClubId = clubId; const club = getClubById(clubId); 
    if (!club) 
        return; 
...
async function handleApplicationSubmit(e) {
    ...
    try {
        ...
        closeClubApplicationModal(); openClubInfoModal(currentClubId); 
...
function searchAndFilterClubs() {
    ...
    if (searchQuery) { const lower = searchQuery.toLowerCase(); 
        clubsToFilter = clubsToFilter.filter(club => club.name.toLowerCase().includes(lower) || club.description.toLowerCase().includes(lower) || club.category.toLowerCase().includes(lower)); 
    }

    if (currentSortBy === 'newest') clubsToFilter.sort((a,b)=> new Date(b.createdDate)-new Date(a.createdDate)); 
    else if (currentSortBy === 'oldest') clubsToFilter.sort((a,b)=> new Date(a.createdDate)-new Date(b.createdDate)); 
    else if (currentSortBy === 'alphabetical') clubsToFilter.sort((a,b)=> a.name.localeCompare(b.name)); 
    else if (currentSortBy === 'most-members') clubsToFilter.sort((a,b)=> b.members - a.members); 
    else if (currentSortBy === 'fewest-members') clubsToFilter.sort((a,b)=> a.members - b.members);
...
function sortClubs(sortBy) { 
    ...
    const sortLabels = { 
        'newest':'Newest', 'oldest':'Oldest', 'alphabetical':'Alphabetical' 
    }; 
...
async function fetchAndInitializeClubs() {
    try {
        ...
        if (studentId) {
            try { const appResponse = await fetch(`http://localhost:3001/api/applications/my-applications/${studentId}`); 
        ...
        if (grid) grid.innerHTML = '<p>Error loading clubs. Please try refreshing.</p>'; 
    }
}